...
metal::float4 transformPositionToView (metal::float3 arg1, device const InstanceObjectState_bufferBlock* InstanceObjectState, device const ObjectState_block* ObjectState, unsigned int VertexStage_sve_instanceID, device const CameraState_block* CameraState);
...
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], unsigned int VertexStage_sve_instanceID [[instance_id]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(1)]], device const CameraState_block* CameraState [[buffer(3)]], device const ObjectState_block* ObjectState [[buffer(0)]]);
metal::float3 cameraWorldPosition (device const CameraState_block* CameraState)
{
...
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], unsigned int VertexStage_sve_instanceID [[instance_id]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(1)]], device const CameraState_block* CameraState [[buffer(3)]], device const ObjectState_block* ObjectState [[buffer(0)]])
{
	...
	_g4 = currentObjectColor(VertexStage_sve_instanceID, ObjectState, InstanceObjectState);
	(*VertexOutput_sve_color) = ((*GenericVertexLayout_sve_color) * _g4);
	_g5 = transformPositionToView((*GenericVertexLayout_sve_position), InstanceObjectState, ObjectState, VertexStage_sve_instanceID, CameraState);
	_l_position4 = _g5;
	_g6 = transformVectorToWorld((*GenericVertexLayout_sve_position), VertexStage_sve_instanceID, ObjectState, InstanceObjectState);
