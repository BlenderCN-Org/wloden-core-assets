...
metal::float3 transformNormalToView (metal::float3 arg1, device const ObjectState_block* ObjectState, unsigned int VertexStage_sve_instanceID, device const InstanceObjectState_bufferBlock* InstanceObjectState, device const CameraState_block* CameraState);
...
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(1)]], device const CameraState_block* CameraState [[buffer(3)]], unsigned int VertexStage_sve_instanceID [[instance_id]], device const ObjectState_block* ObjectState [[buffer(0)]]);
metal::float3 cameraWorldPosition (device const CameraState_block* CameraState)
{
...
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(1)]], device const CameraState_block* CameraState [[buffer(3)]], unsigned int VertexStage_sve_instanceID [[instance_id]], device const ObjectState_block* ObjectState [[buffer(0)]])
{
	...
	_g5 = transformPositionToView((*GenericVertexLayout_sve_position), CameraState, VertexStage_sve_instanceID, ObjectState, InstanceObjectState);
	_l_position4 = _g5;
