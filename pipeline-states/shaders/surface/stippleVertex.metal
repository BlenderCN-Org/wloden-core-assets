...
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(2)]], device const CameraState_block* CameraState [[buffer(4)]], device const ObjectState_block* ObjectState [[buffer(1)]], unsigned int VertexStage_sve_instanceID [[instance_id]]);
metal::float3 cameraWorldPosition (device const CameraState_block* CameraState)
{
...
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(2)]], device const CameraState_block* CameraState [[buffer(4)]], device const ObjectState_block* ObjectState [[buffer(1)]], unsigned int VertexStage_sve_instanceID [[instance_id]])
{
	...
	_g4 = currentObjectColor(InstanceObjectState, VertexStage_sve_instanceID, ObjectState);
	(*VertexOutput_sve_color) = ((*GenericVertexLayout_sve_color) * _g4);
	_g5 = transformPositionToView((*GenericVertexLayout_sve_position), ObjectState, VertexStage_sve_instanceID, CameraState, InstanceObjectState);
	_l_position4 = _g5;
