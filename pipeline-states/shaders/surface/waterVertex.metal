...
metal::float3 transformNormalToView (metal::float3 arg1, device const ObjectState_block* ObjectState, device const InstanceObjectState_bufferBlock* InstanceObjectState, unsigned int VertexStage_sve_instanceID, device const CameraState_block* CameraState);
metal::float4 transformVector4ToView (metal::float4 arg1, device const ObjectState_block* ObjectState, unsigned int VertexStage_sve_instanceID, device const CameraState_block* CameraState, device const InstanceObjectState_bufferBlock* InstanceObjectState);
metal::float4 transformPositionToView (metal::float3 arg1, device const CameraState_block* CameraState, device const InstanceObjectState_bufferBlock* InstanceObjectState, unsigned int VertexStage_sve_instanceID, device const ObjectState_block* ObjectState);
...
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], device const ObjectState_block* ObjectState [[buffer(1)]], device const CameraState_block* CameraState [[buffer(4)]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(2)]], unsigned int VertexStage_sve_instanceID [[instance_id]], device const MaterialState_block* MaterialState [[buffer(6)]]);
metal::float3 cameraWorldPosition (device const CameraState_block* CameraState)
{
...
{
	metal::float4 _g2;
	_g2 = transformVector4ToView(metal::float4(arg1, 1.0), ObjectState, VertexStage_sve_instanceID, CameraState, InstanceObjectState);
	return _g2;
}
...
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], device const ObjectState_block* ObjectState [[buffer(1)]], device const CameraState_block* CameraState [[buffer(4)]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(2)]], unsigned int VertexStage_sve_instanceID [[instance_id]], device const MaterialState_block* MaterialState [[buffer(6)]])
{
	...
	_g5 = transformNormalToView(_l_tangent, ObjectState, InstanceObjectState, VertexStage_sve_instanceID, CameraState);
	(*VertexOutput_sve_tangent) = _g5;
	_g6 = transformNormalToView(_l_bitangent, ObjectState, InstanceObjectState, VertexStage_sve_instanceID, CameraState);
	(*VertexOutput_sve_bitangent) = _g6;
	_g7 = transformNormalToView(_l_normal, ObjectState, InstanceObjectState, VertexStage_sve_instanceID, CameraState);
	(*VertexOutput_sve_normal) = _g7;
	_g8 = transformPositionToView(_l_position, CameraState, InstanceObjectState, VertexStage_sve_instanceID, ObjectState);
	_l_position4 = _g8;
