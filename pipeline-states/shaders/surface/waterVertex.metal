...
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], device const MaterialState_block* MaterialState [[buffer(6)]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(2)]], unsigned int VertexStage_sve_instanceID [[instance_id]], device const ObjectState_block* ObjectState [[buffer(1)]], device const CameraState_block* CameraState [[buffer(4)]]);
metal::float3 cameraWorldPosition (device const CameraState_block* CameraState)
{
...
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], device const MaterialState_block* MaterialState [[buffer(6)]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(2)]], unsigned int VertexStage_sve_instanceID [[instance_id]], device const ObjectState_block* ObjectState [[buffer(1)]], device const CameraState_block* CameraState [[buffer(4)]])
{
	...
	_g5 = transformNormalToView(_l_tangent, InstanceObjectState, VertexStage_sve_instanceID, ObjectState, CameraState);
	(*VertexOutput_sve_tangent) = _g5;
	_g6 = transformNormalToView(_l_bitangent, InstanceObjectState, VertexStage_sve_instanceID, ObjectState, CameraState);
	(*VertexOutput_sve_bitangent) = _g6;
	_g7 = transformNormalToView(_l_normal, InstanceObjectState, VertexStage_sve_instanceID, ObjectState, CameraState);
	(*VertexOutput_sve_normal) = _g7;
	_g8 = transformPositionToView(_l_position, CameraState, ObjectState, InstanceObjectState, VertexStage_sve_instanceID);
	_l_position4 = _g8;
