...
fragment _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], device const GlobalLightingState_block* GlobalLightingState [[buffer(4)]], metal::sampler albedoSampler [[sampler(0)]], metal::texture2d<float> albedoTexture [[texture(0)]], device const MaterialState_block* MaterialState [[buffer(5)]]);
metal::float3 cameraWorldPosition (device const CameraState_block* CameraState)
{
