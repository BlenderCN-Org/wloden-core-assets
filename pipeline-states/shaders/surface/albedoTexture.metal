...
fragment _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], device const MaterialState_block* MaterialState [[buffer(5)]], device const GlobalLightingState_block* GlobalLightingState [[buffer(4)]], metal::sampler albedoSampler [[sampler(0)]], metal::texture2d<float> albedoTexture [[texture(0)]]);
metal::float3 cameraWorldPosition (device const CameraState_block* CameraState)
{
...
fragment _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], device const MaterialState_block* MaterialState [[buffer(5)]], device const GlobalLightingState_block* GlobalLightingState [[buffer(4)]], metal::sampler albedoSampler [[sampler(0)]], metal::texture2d<float> albedoTexture [[texture(0)]])
{
	...
	forwardLightingModel(&_l_g16, _l_N, _l_V, (*FragmentInput_sve_position), (_l_albedo * MaterialState->albedo), MaterialState->smoothness, MaterialState->fresnel, GlobalLightingState);
