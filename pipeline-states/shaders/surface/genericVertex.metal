...
metal::float4 transformPositionToView (metal::float3 arg1, device const InstanceObjectState_bufferBlock* InstanceObjectState, unsigned int VertexStage_sve_instanceID, device const CameraState_block* CameraState, device const ObjectState_block* ObjectState);
...
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], unsigned int VertexStage_sve_instanceID [[instance_id]], device const CameraState_block* CameraState [[buffer(4)]], device const ObjectState_block* ObjectState [[buffer(1)]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(2)]]);
metal::float3 cameraWorldPosition (device const CameraState_block* CameraState)
{
...
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], unsigned int VertexStage_sve_instanceID [[instance_id]], device const CameraState_block* CameraState [[buffer(4)]], device const ObjectState_block* ObjectState [[buffer(1)]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(2)]])
{
	...
	_g4 = isCurrentObjectInvisible(ObjectState, VertexStage_sve_instanceID, InstanceObjectState);
	if (_g4)
	{
	...
	_g9 = transformPositionToView((*GenericVertexLayout_sve_position), InstanceObjectState, VertexStage_sve_instanceID, CameraState, ObjectState);
	_l_position4 = _g9;
