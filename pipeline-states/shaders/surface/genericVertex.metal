...
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], unsigned int VertexStage_sve_instanceID [[instance_id]], device const CameraState_block* CameraState [[buffer(4)]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(2)]], device const ObjectState_block* ObjectState [[buffer(1)]]);
metal::float3 cameraWorldPosition (device const CameraState_block* CameraState)
{
...
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], unsigned int VertexStage_sve_instanceID [[instance_id]], device const CameraState_block* CameraState [[buffer(4)]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(2)]], device const ObjectState_block* ObjectState [[buffer(1)]])
{
	...
	_g5 = currentObjectColor(VertexStage_sve_instanceID, ObjectState, InstanceObjectState);
	(*VertexOutput_sve_color) = ((*GenericVertexLayout_sve_color) * _g5);
	(*VertexOutput_sve_texcoord) = (*GenericVertexLayout_sve_texcoord);
	_g6 = transformNormalToView((*GenericVertexLayout_sve_tangent4).xyz, InstanceObjectState, VertexStage_sve_instanceID, CameraState, ObjectState);
	(*VertexOutput_sve_tangent) = _g6;
	_g7 = transformNormalToView((*GenericVertexLayout_sve_normal), InstanceObjectState, VertexStage_sve_instanceID, CameraState, ObjectState);
	...
	_g9 = transformPositionToView((*GenericVertexLayout_sve_position), CameraState, InstanceObjectState, VertexStage_sve_instanceID, ObjectState);
	_l_position4 = _g9;
