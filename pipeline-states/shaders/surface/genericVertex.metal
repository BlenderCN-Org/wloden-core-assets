...
metal::float3 transformNormalToView (metal::float3 arg1, unsigned int VertexStage_sve_instanceID, device const CameraState_block* CameraState, device const InstanceObjectState_bufferBlock* InstanceObjectState, device const ObjectState_block* ObjectState);
...
metal::float4 transformPositionToView (metal::float3 arg1, device const InstanceObjectState_bufferBlock* InstanceObjectState, device const CameraState_block* CameraState, unsigned int VertexStage_sve_instanceID, device const ObjectState_block* ObjectState);
...
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], device const CameraState_block* CameraState [[buffer(4)]], device const ObjectState_block* ObjectState [[buffer(1)]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(2)]], unsigned int VertexStage_sve_instanceID [[instance_id]]);
metal::float3 cameraWorldPosition (device const CameraState_block* CameraState)
{
...
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], device const CameraState_block* CameraState [[buffer(4)]], device const ObjectState_block* ObjectState [[buffer(1)]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(2)]], unsigned int VertexStage_sve_instanceID [[instance_id]])
{
	...
	_g6 = transformNormalToView((*GenericVertexLayout_sve_tangent4).xyz, VertexStage_sve_instanceID, CameraState, InstanceObjectState, ObjectState);
	(*VertexOutput_sve_tangent) = _g6;
	_g7 = transformNormalToView((*GenericVertexLayout_sve_normal), VertexStage_sve_instanceID, CameraState, InstanceObjectState, ObjectState);
	...
	_g9 = transformPositionToView((*GenericVertexLayout_sve_position), InstanceObjectState, CameraState, VertexStage_sve_instanceID, ObjectState);
	_l_position4 = _g9;
