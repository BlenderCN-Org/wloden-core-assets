...
metal::float4 transformVector4ToView (metal::float4 arg1, unsigned int VertexStage_sve_instanceID, device const CameraState_block* CameraState, device const ObjectState_block* ObjectState, device const InstanceObjectState_bufferBlock* InstanceObjectState);
metal::float4 transformPositionToView (metal::float3 arg1, device const CameraState_block* CameraState, unsigned int VertexStage_sve_instanceID, device const ObjectState_block* ObjectState, device const InstanceObjectState_bufferBlock* InstanceObjectState);
...
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], unsigned int VertexStage_sve_instanceID [[instance_id]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(2)]], device const ObjectState_block* ObjectState [[buffer(1)]], device const CameraState_block* CameraState [[buffer(4)]]);
metal::float3 cameraWorldPosition (device const CameraState_block* CameraState)
{
...
{
	metal::float4 _g2;
	_g2 = transformVector4ToView(metal::float4(arg1, 1.0), VertexStage_sve_instanceID, CameraState, ObjectState, InstanceObjectState);
	return _g2;
}
...
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], unsigned int VertexStage_sve_instanceID [[instance_id]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(2)]], device const ObjectState_block* ObjectState [[buffer(1)]], device const CameraState_block* CameraState [[buffer(4)]])
{
	...
	_g6 = transformNormalToView((*GenericVertexLayout_sve_tangent4).xyz, CameraState, VertexStage_sve_instanceID, InstanceObjectState, ObjectState);
	(*VertexOutput_sve_tangent) = _g6;
	_g7 = transformNormalToView((*GenericVertexLayout_sve_normal), CameraState, VertexStage_sve_instanceID, InstanceObjectState, ObjectState);
	...
	_g9 = transformPositionToView((*GenericVertexLayout_sve_position), CameraState, VertexStage_sve_instanceID, ObjectState, InstanceObjectState);
	_l_position4 = _g9;
