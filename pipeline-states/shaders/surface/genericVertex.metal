...
metal::float4 transformPositionToView (metal::float3 arg1, device const InstanceObjectState_bufferBlock* InstanceObjectState, device const ObjectState_block* ObjectState, device const CameraState_block* CameraState, unsigned int VertexStage_sve_instanceID);
...
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(2)]], device const CameraState_block* CameraState [[buffer(4)]], unsigned int VertexStage_sve_instanceID [[instance_id]], device const ObjectState_block* ObjectState [[buffer(1)]]);
metal::float3 cameraWorldPosition (device const CameraState_block* CameraState)
{
...
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(2)]], device const CameraState_block* CameraState [[buffer(4)]], unsigned int VertexStage_sve_instanceID [[instance_id]], device const ObjectState_block* ObjectState [[buffer(1)]])
{
	...
	_g4 = isCurrentObjectInvisible(VertexStage_sve_instanceID, ObjectState, InstanceObjectState);
	if (_g4)
	{
	...
	_g6 = transformNormalToView((*GenericVertexLayout_sve_tangent4).xyz, VertexStage_sve_instanceID, ObjectState, CameraState, InstanceObjectState);
	(*VertexOutput_sve_tangent) = _g6;
	_g7 = transformNormalToView((*GenericVertexLayout_sve_normal), VertexStage_sve_instanceID, ObjectState, CameraState, InstanceObjectState);
	...
	_g9 = transformPositionToView((*GenericVertexLayout_sve_position), InstanceObjectState, ObjectState, CameraState, VertexStage_sve_instanceID);
	_l_position4 = _g9;
