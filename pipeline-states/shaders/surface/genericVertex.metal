...
metal::float3 transformNormalToView (metal::float3 arg1, device const InstanceObjectState_bufferBlock* InstanceObjectState, device const CameraState_block* CameraState, device const ObjectState_block* ObjectState, unsigned int VertexStage_sve_instanceID);
metal::float4 transformVector4ToView (metal::float4 arg1, device const InstanceObjectState_bufferBlock* InstanceObjectState, unsigned int VertexStage_sve_instanceID, device const CameraState_block* CameraState, device const ObjectState_block* ObjectState);
metal::float4 transformPositionToView (metal::float3 arg1, device const CameraState_block* CameraState, unsigned int VertexStage_sve_instanceID, device const InstanceObjectState_bufferBlock* InstanceObjectState, device const ObjectState_block* ObjectState);
...
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], device const CameraState_block* CameraState [[buffer(4)]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(2)]], unsigned int VertexStage_sve_instanceID [[instance_id]], device const ObjectState_block* ObjectState [[buffer(1)]]);
metal::float3 cameraWorldPosition (device const CameraState_block* CameraState)
{
...
{
	metal::float4 _g2;
	_g2 = transformVector4ToView(metal::float4(arg1, 1.0), InstanceObjectState, VertexStage_sve_instanceID, CameraState, ObjectState);
	return _g2;
}
...
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], device const CameraState_block* CameraState [[buffer(4)]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(2)]], unsigned int VertexStage_sve_instanceID [[instance_id]], device const ObjectState_block* ObjectState [[buffer(1)]])
{
	...
	_g4 = isCurrentObjectInvisible(InstanceObjectState, VertexStage_sve_instanceID, ObjectState);
	if (_g4)
	{
		(*VertexStage_sve_screenPosition) = metal::float4(2.0, 2.0, 2.0, 1.0);
		return _slvm_stageout;
	}
	_g5 = currentObjectColor(InstanceObjectState, ObjectState, VertexStage_sve_instanceID);
	(*VertexOutput_sve_color) = ((*GenericVertexLayout_sve_color) * _g5);
	(*VertexOutput_sve_texcoord) = (*GenericVertexLayout_sve_texcoord);
	_g6 = transformNormalToView((*GenericVertexLayout_sve_tangent4).xyz, InstanceObjectState, CameraState, ObjectState, VertexStage_sve_instanceID);
	(*VertexOutput_sve_tangent) = _g6;
	_g7 = transformNormalToView((*GenericVertexLayout_sve_normal), InstanceObjectState, CameraState, ObjectState, VertexStage_sve_instanceID);
	...
	_g9 = transformPositionToView((*GenericVertexLayout_sve_position), CameraState, VertexStage_sve_instanceID, InstanceObjectState, ObjectState);
	_l_position4 = _g9;
