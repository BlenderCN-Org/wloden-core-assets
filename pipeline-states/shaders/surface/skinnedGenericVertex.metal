...
metal::float3 transformNormalToView (metal::float3 arg1, device const ObjectState_block* ObjectState, unsigned int VertexStage_sve_instanceID, device const CameraState_block* CameraState, device const InstanceObjectState_bufferBlock* InstanceObjectState);
metal::float4 transformVector4ToView (metal::float4 arg1, device const CameraState_block* CameraState, device const InstanceObjectState_bufferBlock* InstanceObjectState, unsigned int VertexStage_sve_instanceID, device const ObjectState_block* ObjectState);
...
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], device const CameraState_block* CameraState [[buffer(4)]], unsigned int VertexStage_sve_instanceID [[instance_id]], device const PoseState_bufferBlock* PoseState [[buffer(3)]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(2)]], device const ObjectState_block* ObjectState [[buffer(1)]]);
metal::float3 cameraWorldPosition (device const CameraState_block* CameraState)
{
...
{
	metal::float4 _g2;
	_g2 = transformVector4ToView(metal::float4(arg1, 1.0), CameraState, InstanceObjectState, VertexStage_sve_instanceID, ObjectState);
	return _g2;
}
...
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], device const CameraState_block* CameraState [[buffer(4)]], unsigned int VertexStage_sve_instanceID [[instance_id]], device const PoseState_bufferBlock* PoseState [[buffer(3)]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(2)]], device const ObjectState_block* ObjectState [[buffer(1)]])
{
	...
	_g21 = transformNormalToView(_g20, ObjectState, VertexStage_sve_instanceID, CameraState, InstanceObjectState);
	...
	_g23 = transformNormalToView(_g22, ObjectState, VertexStage_sve_instanceID, CameraState, InstanceObjectState);
