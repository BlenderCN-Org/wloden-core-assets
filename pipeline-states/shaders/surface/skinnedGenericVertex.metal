...
metal::float3 transformNormalToView (metal::float3 arg1, device const InstanceObjectState_bufferBlock* InstanceObjectState, device const ObjectState_block* ObjectState, unsigned int VertexStage_sve_instanceID, device const CameraState_block* CameraState);
metal::float4 transformVector4ToView (metal::float4 arg1, device const ObjectState_block* ObjectState, device const InstanceObjectState_bufferBlock* InstanceObjectState, unsigned int VertexStage_sve_instanceID, device const CameraState_block* CameraState);
metal::float4 transformPositionToView (metal::float3 arg1, device const ObjectState_block* ObjectState, device const CameraState_block* CameraState, unsigned int VertexStage_sve_instanceID, device const InstanceObjectState_bufferBlock* InstanceObjectState);
...
metal::float3 skinVector (metal::float3 arg1, thread metal::float4* SkinnedGenericVertexLayout_sve_boneWeights, device const PoseState_bufferBlock* PoseState, thread metal::int4* SkinnedGenericVertexLayout_sve_boneIndices);
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], device const PoseState_bufferBlock* PoseState [[buffer(3)]], device const CameraState_block* CameraState [[buffer(4)]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(2)]], device const ObjectState_block* ObjectState [[buffer(1)]], unsigned int VertexStage_sve_instanceID [[instance_id]]);
metal::float3 cameraWorldPosition (device const CameraState_block* CameraState)
{
...
{
	metal::float4 _g2;
	_g2 = transformVector4ToView(metal::float4(arg1, 1.0), ObjectState, InstanceObjectState, VertexStage_sve_instanceID, CameraState);
	return _g2;
}
...
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], device const PoseState_bufferBlock* PoseState [[buffer(3)]], device const CameraState_block* CameraState [[buffer(4)]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(2)]], device const ObjectState_block* ObjectState [[buffer(1)]], unsigned int VertexStage_sve_instanceID [[instance_id]])
{
	...
	_g20 = skinVector((*SkinnedGenericVertexLayout_sve_tangent4).xyz, SkinnedGenericVertexLayout_sve_boneWeights, PoseState, SkinnedGenericVertexLayout_sve_boneIndices);
	_g21 = transformNormalToView(_g20, InstanceObjectState, ObjectState, VertexStage_sve_instanceID, CameraState);
	(*VertexOutput_sve_tangent) = _g21;
	_g22 = skinVector((*SkinnedGenericVertexLayout_sve_normal), SkinnedGenericVertexLayout_sve_boneWeights, PoseState, SkinnedGenericVertexLayout_sve_boneIndices);
	_g23 = transformNormalToView(_g22, InstanceObjectState, ObjectState, VertexStage_sve_instanceID, CameraState);
	...
	_g26 = transformPositionToView(_g25, ObjectState, CameraState, VertexStage_sve_instanceID, InstanceObjectState);
	_l_position4 = _g26;
