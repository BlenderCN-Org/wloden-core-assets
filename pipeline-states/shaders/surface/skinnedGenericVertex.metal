...
metal::float4 transformVector4ToView (metal::float4 arg1, device const ObjectState_block* ObjectState, unsigned int VertexStage_sve_instanceID, device const InstanceObjectState_bufferBlock* InstanceObjectState, device const CameraState_block* CameraState);
metal::float4 transformPositionToView (metal::float3 arg1, device const ObjectState_block* ObjectState, unsigned int VertexStage_sve_instanceID, device const CameraState_block* CameraState, device const InstanceObjectState_bufferBlock* InstanceObjectState);
...
metal::float3 transformVectorToWorld (metal::float3 arg1, device const ObjectState_block* ObjectState, unsigned int VertexStage_sve_instanceID, device const InstanceObjectState_bufferBlock* InstanceObjectState);
...
metal::float3 skinVector (metal::float3 arg1, thread metal::float4* SkinnedGenericVertexLayout_sve_boneWeights, thread metal::int4* SkinnedGenericVertexLayout_sve_boneIndices, device const PoseState_bufferBlock* PoseState);
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(2)]], unsigned int VertexStage_sve_instanceID [[instance_id]], device const PoseState_bufferBlock* PoseState [[buffer(3)]], device const ObjectState_block* ObjectState [[buffer(1)]], device const CameraState_block* CameraState [[buffer(4)]]);
metal::float3 cameraWorldPosition (device const CameraState_block* CameraState)
{
...
{
	metal::float4 _g2;
	_g2 = transformVector4ToView(metal::float4(arg1, 1.0), ObjectState, VertexStage_sve_instanceID, InstanceObjectState, CameraState);
	return _g2;
}
...
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(2)]], unsigned int VertexStage_sve_instanceID [[instance_id]], device const PoseState_bufferBlock* PoseState [[buffer(3)]], device const ObjectState_block* ObjectState [[buffer(1)]], device const CameraState_block* CameraState [[buffer(4)]])
{
	...
	_g20 = skinVector((*SkinnedGenericVertexLayout_sve_tangent4).xyz, SkinnedGenericVertexLayout_sve_boneWeights, SkinnedGenericVertexLayout_sve_boneIndices, PoseState);
	_g21 = transformNormalToView(_g20, VertexStage_sve_instanceID, InstanceObjectState, ObjectState, CameraState);
	(*VertexOutput_sve_tangent) = _g21;
	_g22 = skinVector((*SkinnedGenericVertexLayout_sve_normal), SkinnedGenericVertexLayout_sve_boneWeights, SkinnedGenericVertexLayout_sve_boneIndices, PoseState);
	_g23 = transformNormalToView(_g22, VertexStage_sve_instanceID, InstanceObjectState, ObjectState, CameraState);
	...
	_g26 = transformPositionToView(_g25, ObjectState, VertexStage_sve_instanceID, CameraState, InstanceObjectState);
	_l_position4 = _g26;
