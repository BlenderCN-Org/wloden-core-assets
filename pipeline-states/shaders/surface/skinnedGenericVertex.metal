...
metal::float3 transformNormalToView (metal::float3 arg1, device const InstanceObjectState_bufferBlock* InstanceObjectState, unsigned int VertexStage_sve_instanceID, device const ObjectState_block* ObjectState, device const CameraState_block* CameraState);
metal::float4 transformVector4ToView (metal::float4 arg1, device const InstanceObjectState_bufferBlock* InstanceObjectState, device const ObjectState_block* ObjectState, device const CameraState_block* CameraState, unsigned int VertexStage_sve_instanceID);
metal::float4 transformPositionToView (metal::float3 arg1, unsigned int VertexStage_sve_instanceID, device const InstanceObjectState_bufferBlock* InstanceObjectState, device const CameraState_block* CameraState, device const ObjectState_block* ObjectState);
...
metal::float3 transformVectorToWorld (metal::float3 arg1, unsigned int VertexStage_sve_instanceID, device const InstanceObjectState_bufferBlock* InstanceObjectState, device const ObjectState_block* ObjectState);
...
bool isCurrentObjectInvisible (device const InstanceObjectState_bufferBlock* InstanceObjectState, unsigned int VertexStage_sve_instanceID, device const ObjectState_block* ObjectState);
metal::float3 skinPosition (metal::float3 arg1, thread metal::float4* SkinnedGenericVertexLayout_sve_boneWeights, thread metal::int4* SkinnedGenericVertexLayout_sve_boneIndices, device const PoseState_bufferBlock* PoseState);
metal::float3 skinVector (metal::float3 arg1, device const PoseState_bufferBlock* PoseState, thread metal::float4* SkinnedGenericVertexLayout_sve_boneWeights, thread metal::int4* SkinnedGenericVertexLayout_sve_boneIndices);
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], device const ObjectState_block* ObjectState [[buffer(1)]], device const PoseState_bufferBlock* PoseState [[buffer(3)]], unsigned int VertexStage_sve_instanceID [[instance_id]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(2)]], device const CameraState_block* CameraState [[buffer(4)]]);
metal::float3 cameraWorldPosition (device const CameraState_block* CameraState)
{
...
{
	metal::float4 _g2;
	_g2 = transformVector4ToView(metal::float4(arg1, 1.0), InstanceObjectState, ObjectState, CameraState, VertexStage_sve_instanceID);
	return _g2;
}
...
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], device const ObjectState_block* ObjectState [[buffer(1)]], device const PoseState_bufferBlock* PoseState [[buffer(3)]], unsigned int VertexStage_sve_instanceID [[instance_id]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(2)]], device const CameraState_block* CameraState [[buffer(4)]])
{
	...
	_g20 = skinVector((*SkinnedGenericVertexLayout_sve_tangent4).xyz, PoseState, SkinnedGenericVertexLayout_sve_boneWeights, SkinnedGenericVertexLayout_sve_boneIndices);
	_g21 = transformNormalToView(_g20, InstanceObjectState, VertexStage_sve_instanceID, ObjectState, CameraState);
	(*VertexOutput_sve_tangent) = _g21;
	_g22 = skinVector((*SkinnedGenericVertexLayout_sve_normal), PoseState, SkinnedGenericVertexLayout_sve_boneWeights, SkinnedGenericVertexLayout_sve_boneIndices);
	_g23 = transformNormalToView(_g22, InstanceObjectState, VertexStage_sve_instanceID, ObjectState, CameraState);
	...
	_g25 = skinPosition((*SkinnedGenericVertexLayout_sve_position), SkinnedGenericVertexLayout_sve_boneWeights, SkinnedGenericVertexLayout_sve_boneIndices, PoseState);
	_g26 = transformPositionToView(_g25, VertexStage_sve_instanceID, InstanceObjectState, CameraState, ObjectState);
	_l_position4 = _g26;
