...
metal::float3 transformNormalToView (metal::float3 arg1, device const ObjectState_block* ObjectState, device const CameraState_block* CameraState, unsigned int VertexStage_sve_instanceID, device const InstanceObjectState_bufferBlock* InstanceObjectState);
metal::float4 transformVector4ToView (metal::float4 arg1, device const ObjectState_block* ObjectState, device const CameraState_block* CameraState, unsigned int VertexStage_sve_instanceID, device const InstanceObjectState_bufferBlock* InstanceObjectState);
metal::float4 transformPositionToView (metal::float3 arg1, device const ObjectState_block* ObjectState, device const InstanceObjectState_bufferBlock* InstanceObjectState, unsigned int VertexStage_sve_instanceID, device const CameraState_block* CameraState);
...
metal::float3 skinVector (metal::float3 arg1, thread metal::int4* SkinnedGenericVertexLayout_sve_boneIndices, thread metal::float4* SkinnedGenericVertexLayout_sve_boneWeights, device const PoseState_bufferBlock* PoseState);
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], device const CameraState_block* CameraState [[buffer(3)]], device const PoseState_bufferBlock* PoseState [[buffer(2)]], unsigned int VertexStage_sve_instanceID [[instance_id]], device const ObjectState_block* ObjectState [[buffer(0)]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(1)]]);
metal::float3 cameraWorldPosition (device const CameraState_block* CameraState)
{
...
{
	metal::float4 _g2;
	_g2 = transformVector4ToView(metal::float4(arg1, 1.0), ObjectState, CameraState, VertexStage_sve_instanceID, InstanceObjectState);
	return _g2;
}
...
vertex _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], device const CameraState_block* CameraState [[buffer(3)]], device const PoseState_bufferBlock* PoseState [[buffer(2)]], unsigned int VertexStage_sve_instanceID [[instance_id]], device const ObjectState_block* ObjectState [[buffer(0)]], device const InstanceObjectState_bufferBlock* InstanceObjectState [[buffer(1)]])
{
	...
	_g20 = skinVector((*SkinnedGenericVertexLayout_sve_tangent4).xyz, SkinnedGenericVertexLayout_sve_boneIndices, SkinnedGenericVertexLayout_sve_boneWeights, PoseState);
	_g21 = transformNormalToView(_g20, ObjectState, CameraState, VertexStage_sve_instanceID, InstanceObjectState);
	(*VertexOutput_sve_tangent) = _g21;
	_g22 = skinVector((*SkinnedGenericVertexLayout_sve_normal), SkinnedGenericVertexLayout_sve_boneIndices, SkinnedGenericVertexLayout_sve_boneWeights, PoseState);
	_g23 = transformNormalToView(_g22, ObjectState, CameraState, VertexStage_sve_instanceID, InstanceObjectState);
	...
	_g26 = transformPositionToView(_g25, ObjectState, InstanceObjectState, VertexStage_sve_instanceID, CameraState);
	_l_position4 = _g26;
