...
layout ( SLVM_GL_BINDING_VK_SET_BINDING(6, 3, 0), std140 ) uniform MaterialState_block
{
	vec4 albedo;
	vec3 fresnel;
	float smoothness;
...
layout ( SLVM_GL_BINDING_VK_SET_BINDING(8, 2, 0), std140 ) uniform GlobalLightingState_block
{
	vec4 groundLighting;
	vec4 skyLighting;
	vec3 sunDirection;
...
void main ()
{
	...
	_l_fresnel = _g1.xyz;
	forwardLightingModel(_l_g23, _l_N, _l_V, FragmentInput_sve_position, (_l_albedo * MaterialState.albedo), MaterialState.smoothness, _l_fresnel);
