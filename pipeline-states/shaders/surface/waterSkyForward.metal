...
fragment _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], metal::texturecube<float> skyTexture [[texture(3)]], device const CameraState_block* CameraState [[buffer(3)]], device const MaterialState_block* MaterialState , metal::sampler skySampler [[sampler(2)]], device const GlobalLightingState_block* GlobalLightingState [[buffer(4)]]);
metal::float3 cameraWorldPosition (device const CameraState_block* CameraState)
{
