...
fragment _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], metal::texturecube<float> skyTexture [[texture(3)]], device const MaterialState_block* MaterialState , device const CameraState_block* CameraState [[buffer(3)]], metal::sampler skySampler [[sampler(2)]], device const GlobalLightingState_block* GlobalLightingState [[buffer(4)]]);
metal::float3 cameraWorldPosition (device const CameraState_block* CameraState)
{
...
fragment _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], metal::texturecube<float> skyTexture [[texture(3)]], device const MaterialState_block* MaterialState , device const CameraState_block* CameraState [[buffer(3)]], metal::sampler skySampler [[sampler(2)]], device const GlobalLightingState_block* GlobalLightingState [[buffer(4)]])
{
	metal::float3 _l_N;
	metal::float3 _l_V;
	metal::float4 _l_lightedColor;
	metal::float4 _l_g8;
	metal::float3 _l_R;
	metal::float3 _l_skyPosition;
	metal::float4 _l_skyColor;
	float _l_NdotV;
	metal::float3 _l_reflectionFresnel;
	...
	_l_R = metal::reflect(-_l_V, _l_N);
	_g3 = (CameraState->inverseViewMatrix * metal::float4(_l_R, 0.0));
	_l_skyPosition = _g3.xyz;
	_l_skyColor = skyTexture.sample(skySampler, _l_skyPosition);
	_l_NdotV = metal::max(0.0, metal::min(1.0, metal::dot(_l_N, _l_V)));
	...
	(*FragmentOutput_sve_color) = metal::float4(((_l_skyColor.xyz * _l_reflectionFresnel) + _l_lightedColor.xyz), _l_reflectionFactor);
