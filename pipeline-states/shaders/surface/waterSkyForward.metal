...
fragment _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], metal::sampler skySampler [[sampler(2)]], device const GlobalLightingState_block* GlobalLightingState [[buffer(4)]], device const MaterialState_block* MaterialState , device const CameraState_block* CameraState [[buffer(3)]], metal::texturecube<float> skyTexture [[texture(3)]]);
metal::float3 cameraWorldPosition (device const CameraState_block* CameraState)
{
