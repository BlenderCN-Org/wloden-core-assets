...
fragment _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], device const GlobalLightingState_block* GlobalLightingState [[buffer(4)]], metal::texturecube<float> skyTexture [[texture(1)]], metal::sampler skySampler [[sampler(2)]], device const CameraState_block* CameraState [[buffer(3)]], device const MaterialState_block* MaterialState [[buffer(5)]]);
metal::float3 cameraWorldPosition (device const CameraState_block* CameraState)
{
