...
fragment _SLVM_ShaderStageOutput shaderMain (_SLVM_ShaderStageInput _slvm_stagein [[stage_in]], device const GlobalLightingState_block* GlobalLightingState [[buffer(4)]], device const MaterialState_block* MaterialState );
metal::float3 cameraWorldPosition (device const CameraState_block* CameraState)
{
